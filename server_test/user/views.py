...
def register_user(request):
    if request.method == 'POST':
        ...
        decode_result, data = decoder._decode(request, ['name', 'last_name', 'phone', 'password'])
        # decoding success
        if decode_result:
            # try to create new user
            user_creating_result, user_creating_message = NewUser.manager.create_user(name=data.get('name', None),
